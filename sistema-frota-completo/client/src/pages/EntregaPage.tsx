...
export default function EntregaPage() {
  ...
  const deleteDeliveryMutation = useMutation({
    ...
    onError: (error: any) => {
      toast.error("Erro ao excluir entrega: " + (error.response?.data?.detail || error.message));
      setDeleteConfirmId(null);
    }
  });
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200">
                  <th className="px-6 py-4 font-semibold text-slate-700">Pedido</th>
                  <th className="px-6 py-4 font-semibold text-slate-700">Cliente</th>
                  <th className="px-6 py-4 font-semibold text-slate-700">Status</th>
                  <th className="px-6 py-4 font-semibold text-slate-700">Data de Entrega</th>
              ...
              <tbody>
                {isLoading ? (
                  ...
                  filteredDeliveries.map((delivery: any) => (
                    <tr key={delivery.id} className="border-b border-slate-100 hover:bg-slate-50/50 transition-colors">
                      <td className="px-6 py-4 font-bold text-slate-900">#{delivery.ordernumber}</td>
                      ...
                      <td className="px-6 py-4">
                        <Badge className={`${getStatusColor(delivery.status)} border-none shadow-none`}>
                          {delivery.status == "in_progress" ? "Em rota" : delivery.status == "entregue" ? "Entregue" : "erro"}
                        </Badge>
                      </td>
        ...
        <AlertDialog open={deleteConfirmId !== null} onOpenChange={() => setDeleteConfirmId(null)}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Esta ação removerá o registro de entrega. Se o pedido estiver vinculado a uma rota ativa, você poderá removê-lo da rota após esta exclusão.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel>Cancelar</AlertDialogCancel>
              <AlertDialogAction className="bg-red-600 hover:bg-red-700" onClick={() => deleteConfirmId && deleteDeliveryMutation.mutate(deleteConfirmId)}>
                Excluir
              </AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
