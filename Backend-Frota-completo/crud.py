...
def update_route_item_for_id(
    ...
    db_item = (
        ...
        .filter(models.RouteItem.id == id)
        .first()
    )

    if not db_item:
        print("PULOU ATT ROUTE ITEM")
        return None
    ...
    # print("AAAAAAAAAAAAAAAAAAA: "+ db_item.ordernumber)
    try:
        if(db_item.ordernumber == "" or db_item.ordernumber is None):
            if new_status is not None:
                routeItem = (
                    db.query(models.RouteItem)
                    .filter(
                        models.RouteItem.routeid == db_item.routeid
                    )
                    .first()
                )
                print(routeItem.reason)
                if routeItem:
                    routeItem.status = new_status
